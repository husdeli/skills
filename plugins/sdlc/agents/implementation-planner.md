...
2. **Check the product docs** — read `.sdlc/prd.md` for product context. Then list `.sdlc/designs/*.design.md` and read the design docs whose subject this task touches, for how the solution is supposed to work — its parts, flows, and behavior. A project on an older shape keeps its design docs directly in `.sdlc/`, or in one `design.md`; read those instead. When that folder does not exist, fall back to the project root (`prd.md`/`PRD.md`, `*.design.md`, `design.md`).
