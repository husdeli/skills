...
- Design docs live in **`.sdlc/designs/`**, one file per subject, named `<subject>.design.md` (`checkout.design.md`, `event-ingestion.design.md`, `app-shell.design.md`). Create the folder if it is missing, or run `/scaffold` first when the project has no structure at all.
- **Look before you write.** List `.sdlc/designs/*.design.md`. When a doc for this target already exists, update it in place. When the project keeps its design docs on an older shape — `<subject>.design.md` files directly in `.sdlc/`, or a single `design.md` there or at the root — update the file where it already sits, instead of starting a parallel convention beside it.
