...
```
  ...
  (settle Decisions)                                implement ─► verify ─► [fix ×2]
                                          ◄──────  { status, ... }
...
- **Verify** — verification commands run concurrently; on failure the script loops failures back to `coding` (**at most 2 fix cycles**), then re-verifies. Persistent failure → `status: "escalate"`.
...
- **`status: "escalate"`** → the script hit its limit (plan still rejected after 1 revision, coding blocker, or verification still failing after 2 fix cycles). Surface `stage` + `reason` + details to the user. Leave status `In Progress`; never mark Completed.
