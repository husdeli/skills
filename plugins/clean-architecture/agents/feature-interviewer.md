...
1. **Read the product docs** — they live in **`.clean-architecture/`**: `prd.md` for product intent, users, and constraints; one `<subject>.design.md` per subject for how the solution is supposed to work — its parts, flows, states, and behavior. List `*.design.md` and read the ones this feature touches. A project on the older shape keeps a single `design.md`; read that instead. When that folder does not exist, fall back to the project root (`prd.md`/`PRD.md`, `*.design.md`, `design.md`). Note where the task **diverges from, extends, or contradicts** these docs — contradictions are one of the few things worth raising.
