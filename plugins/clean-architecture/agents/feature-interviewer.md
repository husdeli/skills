...
1. **Read the product docs** — they live in **`.clean-architecture/`**: `prd.md` for product intent, users, and constraints; one `designs/<subject>.design.md` per subject for how the solution is supposed to work — its parts, flows, states, and behavior. List `designs/*.design.md` and read the ones this feature touches. A project on an older shape keeps its design docs directly in `.clean-architecture/`, or in a single `design.md`; read those instead. When that folder does not exist, fall back to the project root (`prd.md`/`PRD.md`, `*.design.md`, `design.md`). Note where the task **diverges from, extends, or contradicts** these docs — contradictions are one of the few things worth raising.
...
- **Genuinely unclear intent** — the request is ambiguous in a way you cannot resolve from the product docs, project instructions, or codebase precedent, and guessing wrong would waste real work.
