...
2. **Load the relevant skills before writing code** — you have the `Skill` tool; use it. Skill names may be namespaced in this environment (e.g. `clean-architecture:clean-fullstack-architecture`); invoke the namespaced form when present.
   ...
   - **Before writing or editing any TypeScript file** (`.ts`/`.tsx`, of any kind — service, domain module, utility, component), invoke the **`ts-clean`** skill and follow its rules — one module per file named after its primary export, static imports at the top of the file (no `await import()`/`require()` inside a function unless it is one of the listed exceptions, with a one-line why), self-documenting code instead of comments that restate it, and configuration values in `.config.ts` modules that are the only place `process.env` is read — required variables validated at load and throwing by name, optional ones given an explicit typed default, secrets never defaulted.
   ...
   - **Before writing or editing any TanStack Start server code** (a file defining, importing, or calling a `createServerFn` server function, or any module holding DB/secret access in a Start app), invoke the **`clean-tanstack-start`** skill *on top of `ts-clean`* and follow its rules — the `.functions.ts` / `.server.ts` / plain `.ts` split, server-only modules never imported from client-reachable files, configuration split the same way (client-safe values in `*.config.ts`, secrets and internal targets in `*.config.server.ts`, which server code may read from but never the reverse), server functions imported statically only (never `await import()`, whatever the bundle-size argument), auth enforced in every handler that touches private data rather than in the calling route's `beforeLoad`, and no `Cache-Control: public` on an identity-dependent response.
...
- **Obey the plugin's skills.** The `clean-fullstack-architecture` skill is mandatory whenever you write production code, `ts-clean` whenever you touch a `.ts`/`.tsx` file, `react-clean` whenever you touch a component or hook, and `clean-tanstack-start` whenever you touch TanStack Start server code. Loading and following them is not optional.
